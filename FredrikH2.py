...
class ErikurStower:
    ...
    def __init__(self, packing_list: dict, vehicle: dict) -> None:
        if not isinstance(packing_list, list):   # Kontrollera att det är en dict
            raise ValueError("Ogiltig 'game_info.")

        try: # Testa om rätt keys finns
            self._truck = CyberTruck(vehicle)
            #self._not_loaded_packages =  [Package(p) for p in game_info["dimensions"]]
        except KeyError:
                raise ValueError("Ogiltig 'game_info.")  
        
        self._not_loaded_packages = packing_list
    ...
    def stow_truck(self) -> list:
        ...
        return (solution, self._not_loaded_packages, f"Paket nr {self.random_index} bytte plats med paket nr {self.random_index+self.test1}\n... och paket nr {self.random_index2} bytte plats med paket nr {self.random_index2+self.test2}")
